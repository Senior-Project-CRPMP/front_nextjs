...
const Chat: React.FC = () => {
  ...
  const fetchProjectMembers = async () => {
    try {
      ...
      const chatRoomsPromises = otherMembers.map(member => fetchChatRoomsWithUser(member.id));

      const fetchedChatRooms = await Promise.all(chatRoomsPromises);
      const uniqueChatRooms = Array.from(new Set(fetchedChatRooms.flat().map(room => room.chatRoomId)))
        .map(id => fetchedChatRooms.flat().find(room => room.chatRoomId === id))
  ...
  const fetchChatRoomsWithUser = async (memberId: string) => {
    try {
      const response = await fetch(`${apiBaseUrl}/api/ChatRoom/chatrooms/with-users?userIds=${userId}&userIds=${memberId}`);
  ...
  const fetchMessages = async (roomId: number) => {
    try {
      const response = await fetch(`${apiBaseUrl}/api/ChatRoom/chatroom/${roomId}/messages`);
  ...
  const sendMessage = async () => {
    try {
      const response = await fetch(`${apiBaseUrl}/api/ChatRoom/chatroom/${currentRoom}/messages/create`, {
        method: "POST",
        headers: {
    ...
    <div className="flex">
      ...
      <div className="flex-1 flex flex-col items-center justify-center">
        {!chatRooms || chatRooms.length === 0 ? (
          <div className="max-w-sm bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700 h-52 flex items-center justify-center">
              ...
              <ul>
                {chatRooms.map((room) => (
                  <li key={room.chatRoomId}>
                    <button className="w-full p-1 m-1 bg-white" onClick={() => joinChatRoom(room.chatRoomId)}>
                      {room.name}
                    </button>
                  </li>
                ))}
