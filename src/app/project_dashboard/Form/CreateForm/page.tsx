...
const AddForm: React.FC = () => {
    ...
    const handleFieldChange = (index: number, name: keyof FormField, value: any, optionIndex?: number | boolean) => {
        const updatedFields = [...formFields];
    ...
    const handleSubmit = async (event: FormEvent) => {
        ...
        try {
            const formResponse = await fetch("http://localhost:5110/api/Form/CreateForm", {
                method: 'POST',
                headers: {
            ...
            const createQuestionPromises = formFields.map(async (field) => {
                ...
                const questionResponse = await fetch("http://localhost:5110/api/FormQuestion/CreateQuestion", {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                    },
                ...
                if (field.type === "select" || field.type === "checkbox") {
                    const createOptionPromises = field.options.map(async (option) => {
                        await fetch("http://localhost:5110/api/FormOption/CreateOption", {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json',
                            },
                    ...
                    <div key={fieldIndex}>
                        <label>
                            ...
                            <input
                                ...
                                value={field.label}
                                onChange={(e) =>
                                    handleFieldChange(fieldIndex, e.target.name as keyof FormField, e.target.value)
                                }
                            />
                        </label>
                        <label>
                        ...
                        <label>
                            ...
                            <input
                                ...
                                onChange={(e) =>
                                    handleFieldChange(fieldIndex, e.target.name as keyof FormField, e.target.checked)
                                }
                            />
                        </label>
                        <label>
                            ...
                            <>
                                ...
                                <label>
                                    ...
                                    <input
                                        ...
                                        onChange={(e) =>
                                            handleFieldChange(fieldIndex, e.target.name as keyof FormField, parseInt(e.target.value))
                                        }
                                    />
                                </label>
                            </>
